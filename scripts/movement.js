...
system.runInterval(() => {
    world.getAllPlayers().forEach(player => {
        ...
        if (playersCoords.has(player.name)) {
            ...
            if (coords2.x !== coords.x || coords2.y !== coords.y || coords2.z !== coords.z) {
                playersCoords.set(player.name, coords);
                console.log("PlayerMovement:" + player.name + " Position:" + getPostion(player.location, player.dimension));
            }
        } else {
            playersCoords.set(player.name, coords);
            console.log("PlayerMovement:" + player.name + " Position:" + getPostion(player.location, player.dimension));
